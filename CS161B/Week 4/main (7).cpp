...
void readInput(char courseNums[][MAXCHAR], int students[], int &count) {
  ...
  // May need to add in if else under !count
  while (strcmp(tempName, "Quit") != 0 && strcmp(tempName, "quit") != 0){
   // readInt("Number of students enrolled: ", students[count]); // added back students[count]

    if(count == CAP) {
      cout << "Array is full!" << endl;
      // return; // else should be used instead of return
     //else until line 134
    } else { // added else
    if(!count) {
      strcpy(courseNums[count], tempName);
      readInt ("Number of students enrolled: ", students[i]); // just added 2/6 per Shawli
      // students[count] = num;
    } else {
      ...
      // students[i] = num;
      readInt ("Number of students enrolled: ", students[count]); // just added 2/6 per Shawli
    } // closing if-else on line 118
    count++;
    cin.ignore(100, '\n');
    cout << "Enter course number: ";
    cin.getline(tempName, MAXCHAR);
    // readInt ("Number of students enrolled: ", students[count]); // JUST ADDED FOR TEST
    }
    }
}
